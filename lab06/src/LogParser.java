...
public class LogParser {
    ...
    private void processStartEntry(String[] words) throws ParseException {
        ...
        Function<CustomerID, Customer> createCustomer = (cid) -> {
            Customer r = new Customer(cid);
            return r;
        };
        Customer customer = log.customerByID.computeIfAbsent(customerID, createCustomer);
    ...
    private void processViewEntry(String[] words) throws ParseException {
        ...
        if (words.length != VIEW_NUM_FIELDS) {
            throw new ParseException(inputFile,lineNumber, "View tag with " + words.length + " words");
        }
        Product p = getProduct(words);
        Session s = log.sessionByID.get(words[VIEW_SESSION_ID]);
        s.addViews(new View(p));
    }

    private Product getProduct(String[] words){
        if (log.productByID.containsKey(words[BUY_PRODUCT_ID])) {
            Product p = log.productByID.get(words[BUY_PRODUCT_ID]);
            return p;
        } else {
            Product p = new Product(words[VIEW_PRODUCT_ID], Double.parseDouble(words[VIEW_PRICE]));
            return p;
        }
    }
    ...
    private void processBuyEntry(String[] words) throws ParseException {
        ...
        if (words.length != BUY_NUM_FIELDS) {
            throw new ParseException(inputFile,lineNumber, "Buy tag with " + words.length + " words");
        }

        Session session = log.sessionByID.get(words[VIEW_SESSION_ID]);
        Product p = getProduct(words);
        Purchase purchase = new Purchase(p, Integer.parseInt(words[BUY_QUANTITY]));
        session.addPurchases(purchase);
    }
